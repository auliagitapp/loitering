"""
detector.py — Integrasi YOLOv8 + ByteTrack + visualisasi dua level alert.

Level 1 — LOITERING  : kotak merah + label "! LOITERING [alasan]"
Level 2 — TAMPERING  : kotak magenta berkedip + label "!! TAMPERING [alasan]"
"""
...
class LoiteringDetector:
    ...
    def __init__(self, fps: float, roi: tuple = None):
        ...
        self.frame_count = 0
        self._prev_gray: np.ndarray = None  # untuk optical flow internal
    ...
    def process_frame(self, frame: np.ndarray, current_time: float) -> tuple:
        ...
        # ------------------------------------------------------------------
        # Optical flow magnitude map (untuk tampering motion detection)
        # Dihitung di sini agar bisa di-pass ke TrackManager
        # ------------------------------------------------------------------
        ...
        # ------------------------------------------------------------------
        # Update TrackManager dengan motion map
        # ------------------------------------------------------------------
        self.track_manager.update_tracks(
            track_list, roi_flags, current_time, motion_map
        )
    ...
    def _annotate_frame(self, frame, track_list, roi_flags):
        ...
        for (track_id, bbox), in_roi in zip(track_list, roi_flags):
            ...
            x1, y1, x2, y2 = bbox
            is_critical = person.flags.is_critical()
            color = person.get_status_color()

            # Kotak lebih tebal untuk tampering
            thickness = 4 if is_critical else (3 if person.is_loitering else 2)
            cv2.rectangle(frame, (x1, y1), (x2, y2), color, thickness)

            # Untuk tampering: tambahkan border kedua agar mencolok
            if is_critical:
            ...
            if person.is_loitering:
                ...
                prefix = "!! TAMPERING" if is_critical else "! LOITERING"
                alert_label = f"{prefix} {reason}"

                font_scale = 0.65 if is_critical else 0.6
    ...
    def _collect_events(self, current_time: float) -> list:
        ...
        for person in self.track_manager.get_loitering_persons():
            if self.track_manager.should_send_alert(person):
                level = "TAMPERING" if person.flags.is_critical() else "LOITERING"
                events.append({
