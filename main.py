...
def process_video(
    ...
    print(f"\n{'='*60}")
    print(f"[VIDEO] {video_name}  |  {width}x{height}  |  {fps:.1f} fps  |  {total_frames} frames")
    print(f"{'='*60}")
    ...
    while True:
        ...
        frame_idx += 1
        current_time = frame_idx / fps  # timestamp dalam detik (deterministik untuk video file)
        ...
        if frame_idx % 100 == 0:
            ...
            print(f"  [{pct:5.1f}%] frame {frame_idx}/{total_frames}  |  {elapsed:.1f}s elapsed")
    ...
    loitering_ids = set(p.track_id for p in detector.track_manager.get_loitering_persons())
    loitering_ids.update(
        p.track_id for p in detector.track_manager.archived_tracks.values()
        if p.is_loitering
    )
...
def _draw_gt_overlay(frame: np.ndarray, frame_idx: int, gt_start: int, gt_end: int) -> np.ndarray:
    ...
    if gt_end > 0:
        ...
        cv2.line(frame, (cur_x, bar_y - 2), (cur_x, bar_y + bar_h + 2), (255, 255, 255), 1)
...
def main():
    ...
    parser.add_argument("--annotation", type=str,
                        help="Path ke CSV anotasi Kaggle untuk evaluasi")
    parser.add_argument("--no-display", action="store_true",
                        help="Jangan tampilkan video (lebih cepat)")
    ...
    for vp in video_files:
        ...
        summary = process_video(
            video_path=vp,
            roi=roi,
            set_roi=args.set_roi and len(video_files) == 1,  # set-roi hanya untuk 1 video
            show=show,
