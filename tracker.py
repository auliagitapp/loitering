"""
tracker.py — State management per individu untuk deteksi loitering.

DEFINISI LOITERING (konteks Bank ATM):
  Fokus tunggal = perilaku mondar-mandir (pacing): pergantian arah
  horizontal berulang di dalam zona ATM.

  - Mondar-mandir (pacing) >= PACING_DIRECTION_CHANGE_THRESHOLD kali → pemicu utama
  Heuristik pendukung yang masih aktif (bukan pemicu tampering):
  - Masuk-keluar zona berulang dalam waktu singkat (reentry)
  - Beberapa orang berkerumun di depan satu ATM (crowding)
...
class PersonState:
    ...
    - Deteksi loitering berbasis pacing (mondar-mandir)
    ...
    def _evaluate_flags(self, current_time: float):
        ...
        # PEMICU UTAMA — loitering = mondar-mandir (pacing).
        self.flags.pacing = (
            self.direction_changes >= config.PACING_DIRECTION_CHANGE_THRESHOLD
        )

        # DINONAKTIFKAN — false positive untuk transaksi normal:
        # orang diam input PIN / antri / satpam patroli bukan loitering.
        self.flags.time_exceeded = False
        self.flags.idle_too_long = False

        # Heuristik pendukung (masih aktif). Re-entry: keluar-masuk berulang.
        recent_entries = [
        ...
        # Face covered: heuristik aspect-ratio bbox (lihat _check_face_covered).
        self.flags.face_covered = self._check_face_covered()

        # Crowding: diset dari TrackManager.update_tracks
...
class TrackManager:
    ...
    def update_tracks(self, detections: list, in_roi_flags: list,
        ...
        for (track_id, bbox), in_roi in zip(detections, in_roi_flags):
            ...
            self.active_tracks[track_id].update(bbox, in_roi, current_time)
