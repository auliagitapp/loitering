...
def load_annotations(csv_path: str) -> dict:
    ...
    for fname, ann in annotations.items():
        status = (
            f"loitering frame {ann['loiter_start']}–{ann['loiter_end']}"
            if ann["has_loitering"]
            else "no loitering"
        )
        print(
            f"  {fname:15s}  {ann['total_frames']:5d} frames  {ann['fps']} fps  → {status}"
        )
...
def print_summary(summary: dict):
    ...
    print(
        f"  Loitering terdeteksi : {'YA' if summary.get('loitering_detected') else 'TIDAK'}"
    )
    ...
    if summary.get("gt_has_loitering") is not None:
        ...
        match = (
            "✓ BENAR"
            if summary["gt_has_loitering"] == summary["pred_loitering"]
            else "✗ SALAH"
        )
        ...
        print(
            f"  Prediksi             : {'YA' if summary['pred_loitering'] else 'TIDAK'}  (tanpa anotasi — tidak dievaluasi)"
        )
    print()
